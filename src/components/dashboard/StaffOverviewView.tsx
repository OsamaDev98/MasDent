...
import type { StaffOverviewData } from '@/lib/dal';
...
export default function StaffOverviewView({ data, lang }: Props) {
  const isAr = lang === 'ar';
  ...
  const { appointments, todayStr } = data;
  ...
  return (
    <DashboardShell
      ...
      actions={
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm bg-white/15 border border-white/30 text-white hover:bg-white/25 backdrop-blur-sm transition-all disabled:opacity-60"
        >
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map((s, i) => (
          <motion.div key={s.label} {...fadeUp(i)}>
            <div
              className={`relative bg-white rounded-3xl p-5 border border-slate-100 shadow-lg ${s.glow} hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group cursor-default`}
            >
              <div
                className={`absolute -top-6 -right-6 w-20 h-20 bg-gradient-to-br ${s.gradient} opacity-10 rounded-full blur-xl group-hover:opacity-20 transition-opacity`}
              />
              <div
                className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${s.gradient} flex items-center justify-center shadow-lg mb-4`}
              >
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-6">
        ...
        <motion.div
          {...fadeUp(4)}
          className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden"
        >
            ...
            {today.length === 0 ? (
              <div className="flex flex-col items-center py-16">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {isAr ? 'استمتع بيومك!' : 'Enjoy your day!'}
                </p>
              </div>
            ) : (
              <div className="divide-y divide-slate-50">
                {today.map((a, i) => {
                  const cfg =
                    STATUS_CFG[a.status as keyof typeof STATUS_CFG] ?? STATUS_CFG.pending;
                  return (
                    <motion.div
                      key={a._id}
                    ...
                    >
                      ...
                      <div
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border shrink-0 ${cfg.bg} ${cfg.border} ${cfg.color}`}
                      >
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-50">
            <div>
              <h3 className="font-black text-slate-900">
                {isAr ? 'قائمة الانتظار' : 'Live Queue'}
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                {isAr ? 'المرضى الحاليون' : 'Active patients'}
              </p>
            </div>
            <div
              className={`px-2.5 py-1 rounded-full text-xs font-black ${todayQueue.length > 0 ? 'bg-teal-50 border border-teal-200 text-teal-700' : 'bg-slate-100 text-slate-500'}`}
            >
              {todayQueue.length}
            </div>
          </div>
            ...
            {todayQueue.length === 0 ? (
              <div className="flex flex-col items-center py-10">
                ...
                <p className="text-sm font-bold text-slate-500">
                  {isAr ? 'قائمة الانتظار فارغة' : 'Queue is empty'}
                </p>
              </div>
            ) : (
              todayQueue.map((a, i) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {quickActions.map((a, i) => (
            <motion.div key={a.label} {...fadeUp(7 + i)}>
              ...
              >
                <div
                  className={`absolute -top-6 -right-6 w-20 h-20 bg-gradient-to-br ${a.gradient} opacity-0 group-hover:opacity-10 rounded-full blur-xl transition-opacity`}
                />
                <div
                  className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${a.gradient} shadow-lg ${a.glow} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                >
