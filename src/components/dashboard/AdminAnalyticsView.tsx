...
import type { AnalyticsData } from '@/lib/dal';
  ...
  return (
    <div className="flex items-end gap-2 h-40 pt-4 relative">
      ...
      {data.map((d, i) => {
        ...
        const dayLabel = new Date(d.date).toLocaleDateString(isAr ? 'ar-SA' : 'en-US', {
          weekday: 'short',
        });
        return (
...
export default function AdminAnalyticsView({ data, lang }: Props) {
  const isAr = lang === 'ar';
  ...
  const handleRefresh = useCallback(() => {
    startTransition(() => {
      router.refresh(); // Re-runs the server component, re-fetches DB data
    });
  }, [router]);

  const o = data.overview;
  const monthChange =
    o.lastMonthAppointments > 0
      ? Math.round(
          ((o.thisMonthAppointments - o.lastMonthAppointments) / o.lastMonthAppointments) * 100,
        )
      : 0;
  ...
  const kpi = [
    {
      ...
      sub: isAr
        ? `${o.thisMonthAppointments} هذا الشهر`
        : `${o.thisMonthAppointments} this month`,
      change: monthChange,
    },
    {
      icon: 'group',
      ...
      sub: isAr
        ? `+${o.newPatientsThisMonth} جديد`
        : `+${o.newPatientsThisMonth} new this month`,
    ...
    },
    {
      ...
      value: o.completed,
      label: isAr ? 'مواعيد مكتملة' : 'Completed',
      sub: isAr
    ...
    },
    {
      ...
      value: o.totalStaff,
      label: isAr ? 'أعضاء الفريق' : 'Staff Members',
  ...
  return (
    <DashboardShell
      ...
      subtitle={
        ...
        (isAr ? 'إحصائيات وأداء العيادة' : 'Clinic performance and real-time insights')
      }
      actions={
        <Button
          ...
          disabled={isPending}
          variant="outline"
          className="gap-2 rounded-xl font-semibold bg-white/15 border-white/30 text-white hover:bg-white/25 hover:text-white backdrop-blur-sm disabled:opacity-60"
        >
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {kpi.map((k, i) => (
          <motion.div key={k.label} {...fadeUp(i)}>
            <div
              className={`relative bg-white rounded-3xl p-5 border border-slate-100 shadow-lg ${k.glow} hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group`}
            >
              <div
                className={`absolute -top-6 -right-6 w-20 h-20 bg-gradient-to-br ${k.gradient} opacity-10 rounded-full blur-xl group-hover:opacity-20 transition-opacity`}
              />
              <div
                className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${k.gradient} flex items-center justify-center shadow-lg mb-4`}
              >
              ...
              <div className="flex items-center gap-1.5 mt-3 pt-2.5 border-t border-slate-50">
                {k.change !== null && k.change !== 0 && (
                  <span
                    className={`flex items-center gap-0.5 text-xs font-bold shrink-0 ${k.change > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                  >
                    <span className="material-symbols-outlined text-xs">
                      {k.change > 0 ? 'trending_up' : 'trending_down'}
                    </span>
                    {Math.abs(k.change)}%
                  </span>
                )}
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-5">
        ...
        <motion.div
          {...fadeUp(4)}
          className="lg:col-span-2 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-shadow"
        >
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="font-black text-slate-900 text-base">
                {isAr ? 'مواعيد آخر 7 أيام' : 'Last 7 Days Trend'}
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                {isAr ? 'عدد الحجوزات الجديدة يومياً' : 'Daily new bookings'}
              </p>
            </div>
        ...
        >
          ...
          <p className="text-xs text-slate-400 mb-5">
            {isAr ? 'من إجمالي المواعيد' : `From ${o.totalAppointments} total`}
          </p>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 rounded-full bg-slate-100 mt-1.5 overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{
                        width: `${o.totalAppointments > 0 ? (s.value / o.totalAppointments) * 100 : 0}%`,
                      }}
                      transition={{
                        duration: 0.7,
                        ease: [0.22, 1, 0.36, 1] as [number, number, number, number],
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        >
          <h3 className="font-black text-slate-900 text-base mb-0.5">
            {isAr ? 'الخدمات الأكثر طلباً' : 'Top Services'}
          </h3>
          <p className="text-xs text-slate-400 mb-5">
            {isAr ? 'الخدمات الأعلى حجزاً' : 'Most booked services this period'}
          </p>
          {data.topServices.length === 0 ? (
            <div className="flex flex-col items-center py-10">
              ...
              <p className="text-slate-400 text-sm font-medium">
                {isAr ? 'لا توجد بيانات بعد' : 'No service data yet'}
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {data.topServices.map((s, i) => {
                ...
                const pctVal = Math.round((s.count / maxCount) * 100);
                const colors = [
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 group">
                      <div
                        className={`w-8 h-8 rounded-xl bg-gradient-to-br ${colors[i % colors.length]} flex items-center justify-center text-white text-xs font-black shrink-0 shadow-sm`}
                      >
                        {i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1.5">
                          ...
                          <span
                            className={`text-xs font-black shrink-0 ml-2 px-2 py-0.5 rounded-lg ${bgColors[i % bgColors.length]} ${textColors[i % textColors.length]}`}
                          >
                            {s.count}
                          </span>
                        </div>
        ...
        >
          <h3 className="font-black text-slate-900 text-base mb-0.5">
            {isAr ? 'إحصائيات سريعة' : 'Quick Snapshot'}
          </h3>
          <p className="text-xs text-slate-400 mb-5">
            {isAr ? 'أرقام رئيسية' : 'Key numbers at a glance'}
          </p>
          <div className="space-y-2.5">
            {[
              ...
              {
                icon: 'speed',
                label: isAr ? 'معدل الغياب' : 'No-show Rate',
                value: `${o.noShowRate}%`,
                bg: 'bg-orange-50',
              ...
              <motion.div
                key={s.label}
                className={`flex items-center gap-3 p-3 rounded-2xl border ring-1 ${s.ring} border-transparent hover:border-slate-100 transition-colors group`}
                initial={{ opacity: 0, x: 10 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.5 + i * 0.06 }}
              >
